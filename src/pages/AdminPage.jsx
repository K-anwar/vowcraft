...
export default function AdminPage() {
  ...
  const { config, loading } = useCustomerConfig(slug);
    ...
    >
      ...
      <div className="max-w-4xl mx-auto space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <StatCard icon="📊" label="Tamu Check-in" value={`${Math.round((stats.checkedIn / stats.attending) * 100)}%`} color="#8b5cf6" />
        </div>
            ...
            <div>
              ...
              <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-full bg-green-500 transition-all duration-500 rounded-full"
                  style={{ width: `${(stats.attending / (stats.totalRSVP || 1)) * 100}%` }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
              ...
              <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-full bg-yellow-500 transition-all duration-500 rounded-full"
                  style={{ width: `${(stats.checkedIn / (stats.attending || 1)) * 100}%` }}></div>
              </div>
            </div>
          </div>
        </div>
...
function StatCard({ icon, label, value, color }) {
  return (
    <div className="p-4 rounded-2xl text-center hover:scale-105 transition-transform"
